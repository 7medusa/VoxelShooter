...
void main() {
    vec4 diffuseColor = texture(u_diffuse_map, out_tex_coord);//wenn nötig .rgb dran hängen
    ...
    main_color = vec4((ambient + diffuse) * diffuseColor.xyz + specular + u_emissive, 1.0);
}
